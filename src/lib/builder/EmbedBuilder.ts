...
import { InvalidEmbedColor } from "#lumine/errors";
...
export class EmbedBuilder extends Base<EmbedOptions> {
	...
	setColor(color: ColorResolvable) {
		if (typeof color === "number") {
			if (color < 0x0 || 0xffffff < color) throw new InvalidEmbedColor("invalid color specified");
			this.data.color = color;
		} else {
			if (color.some((colorFragment) => colorFragment < 0 || 255 < colorFragment))
				throw new InvalidEmbedColor("invalid color specified");
